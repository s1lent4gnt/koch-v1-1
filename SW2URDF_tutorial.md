...
1. In the urdf file, add the mujoco compiler tag, for example:
```xml
<robot>
	<mujoco>
...
```
1. Download MuJoCo binaries https://github.com/google-deepmind/mujoco/releases

2. Convert `urdf` to `mjcf`, Run : `./<path_to_mujoco>/bin/compile robot.urdf robot.xml`

3. In `robot.xml`, define actuators for each joints, for example:
...
4. You may have to tune some joint parameters in `robot.xml`, mainly `damping`, `frictionloss`, `kp` and `forcerange`. To attribute default values to all joints, you can use the `default` tag. For example:
```xml
<mujoco>
    <default>
...
2. Check that everything is working : `./<path_to_mujoco>/bin/simulate robot.xml`. You should be able to control the joints.

3. You can then use your robot in a scene, here is an example scene with lighting, a skybox and a groundplane: 
```xml
<mujoco model="scene">
...
4.  Just run `./<path_to_mujoco>/bin/simulate scene.xml` to see your robot in the scene.


## References

1. Mujoco Documentation, "Getting Started with Mujoco," Accessed on: September 13, 2024. [Mujoco Docs](https://mujoco.org/book/modeling.html)
2. SolidWorks to URDF. [YouTube video series](https://youtu.be/Id8zVHrQSlE?si=8yKYVuxUx-b8Cn2-)
3. Steps to convert URDF to MJCF tutorial. [URDF to MJCF](https://github.com/perezjln/gym-lowcostrobot/blob/main/gym_lowcostrobot/assets/urdf_to_mujoco.md) 